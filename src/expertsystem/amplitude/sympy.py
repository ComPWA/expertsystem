...
def _get_graph_group_unique_label(
    ...
    if graph_group:
        ...
        is_names = _get_helicity_particles(graph_group[0], ise)
        fs_names = _get_helicity_particles(graph_group[0], fse)
        label += (
...
class SympyHelicityAmplitudeGenerator:
    ...
    def __init__(self, reaction_result: Result) -> None:
        ...
        self.__graphs = reaction_result.solutions
    ...
    def _generate_partial_decay(
        ...
        out_edge_ids = graph.get_edge_ids_outgoing_from_node(node_id)
        ...
        wigner_d = Wigner.d(
            j=sy.Rational(parent.particle.spin, 2),
            m=sy.Rational(parent.helicity, 2),
            mp=sy.Rational(children[0].helicity - children[0].helicity, 2),
            beta=x,
        )
